    ...
    def post(self, request):
        if request.method=='POST':
            ...
            if details.is_valid():
                ...
                LoginCredentials.objects.create_user(email=email,username=username,phone_number=phone_number,password=password)
                try:
                    ...
                    UserDetails.objects.create(first_name=first_name,last_name=last_name,profile_photo=profile_picture,user_details=user,user_role='patient')
                    Patient.objects.create(age=age,token=token,user_details=user)
                    send_email_after_registration(user.email,token)
                    messages.success(request,"your account created successfullu to verify your account check your email..")
